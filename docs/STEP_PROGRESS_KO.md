...
- 2026-05-07: Windows Mito OHLCV candidate-event 파이프라인 실행 결과 segmentation/robustness 는 PASS 했지만 same ticker/day random event-time benchmark 는 FAIL 했다. Root 입력 기준 `candidate_events=616,494`, random controls 1,232,988, mean edge -0.190468%, median edge -0.024584%, win-rate edge -1.779876% 로 모두 음수다. 기본 root 입력이 과거 rerun 을 함께 읽어 segmentation 출력 308,247건의 2배가 된 것으로 보이므로 exact `candidate_events.csv` 파일 지정 rerun 을 archive 용으로 남기되, OHLCV-only event timing edge 는 현재 기준 rejected 로 본다.
- 다음 우선순위는 Mito exact `candidate_events.csv` 파일 지정으로 random benchmark 를 한 번 더 남겨 중복 합산 가능성을 제거하고, `audit-mito-event-random-benchmark-breakdown` 으로 event_time/month/bucket 별 pocket 을 확인하는 것이다. 그래도 setup backtest 는 아직 금지이며, pocket 이 보여도 fresh holdout 전용 hypothesis 로만 기록한다.
