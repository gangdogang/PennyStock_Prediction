...
- 2026-05-07: Windows `fade_short_june_2025`, `fade_short_june_v2`, `fade_short_3mo` 결과를 `docs/STATUS.md` 에 기록했다. target 5% -> 10% 변경만으로는 June avg pnl 이 거의 변하지 않았고, 3개월 합산도 -$6,746 으로 음수다. 로컬 코드의 `paper_short_stop_pct=15.0` / `FAILED_BREAKOUT` only 변경은 아직 검증 전 diagnostic ablation 이며, 대화 중 나온 `09:45` 이후 진입 지연은 별도 `--min-entry-time 09:45` replay 로 검증해야 한다. decision-grade strategy 변경으로 보지 않는다.
- 2026-05-07: `fade_short` replay branch 가 long bucket 과 달리 `--min-entry-time` / `--entry-setup-states` 를 건너뛰고 short exit 에 long `sell()` fill 을 쓰던 문제를 수정했다. `FillModel.buy_to_cover()` 를 추가해 ask-side cover fill 로 바꾸고, filter/fill 회귀 테스트를 추가했다. 전체 `.venv/bin/python -m pytest -q tests` 기준 `415 passed, 1 skipped`.
...
- 2026-05-07: Mito candidate symbol 4,991개 기준 SEC PIT backfill 을 완료했다. `sec_pit_mito_candidates_20260507` 는 eligible 68,469 / diagnostic-after-cutoff 58,396 / written 68,469 를 기록했고, `filing_tiers_mito_candidates_20260507` 는 classified filings 68,634건 / symbols 2,494개 / gate PASS 를 기록했다. 이 PASS 는 filing inventory 생성만 의미하며 strategy approval 이 아니다.
- 2026-05-07: `audit-mito-candidate-event-filing-tiers` CLI 를 추가했다. exact Mito `candidate_events.csv`, paired `random_events.csv`, SEC `classified_filings.csv` 를 symbol/date 기준으로 join 해 filing tier별 candidate-vs-random edge 와 trap 제외 효과를 진단한다. 산출물은 계속 `decision_grade=False`, `cost_grade=none` 이며, 통과해도 다음 단계 hypothesis inventory 로만 본다.
