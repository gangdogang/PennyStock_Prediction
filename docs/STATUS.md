...
- `audit-mito-ohlcv-1m`, `segment-mito-candidate-events`, `audit-mito-candidate-event-robustness`, `run-mito-event-random-benchmark` CLI 는 `mito0o852/OHLCV-1m` 월별 parquet directory 를 같은 반증 파이프라인으로 처리한다. 경로는 명시적 `--input-root`, `PSR_MITO_OHLCV_1M_PATH`, `PSR_DATA_ROOT/raw/huggingface/mito_ohlcv_1m/data`, repo-local fallback 순서로 해석한다. 산출물은 계속 `decision_grade=False`, `cost_grade=none` 이며 random benchmark 우위 없이는 setup backtest 금지 상태를 해제하지 않는다.
