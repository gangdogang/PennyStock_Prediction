...
- `audit-replay-bucket-robustness` CLI 는 단일 replay bucket 의 total/avg expectancy, 실제 win% 대비 breakeven win%, 월별 일관성, top symbol/date 제거 후 손익, exit reason 분해를 JSON/CSV 로 산출한다. 산출물은 `BLOCKED` / `REJECT` / `PROMISING_DIAGNOSTIC` 를 찍지만 항상 `decision_grade=false` 이며 strategy approval 이 아니다.
...
- `audit-hf-candidate-event-robustness` CLI 는 연도별 `candidate_events.csv` 를 합쳐 top ticker 제거 전/후 event 수, year/event-time/time-bucket 분포, forward outcome 변화를 감사한다. top10 ticker 제거 후 event 수나 outcome 분포가 붕괴하면 CryptoSpartan HF 데이터는 setup backtest 로 승격하지 않는다.
- `run-hf-event-random-benchmark` CLI 는 같은 ticker/day 안에서 deterministic random event-time 을 생성해 candidate event forward outcome 과 비교한다. random 대비 mean/median/win-rate 우위가 없으면 setup backtest 로 넘어가지 않는다. 이 산출물도 `decision_grade=False`, `cost_grade=none` 이다.
- `audit-mito-ohlcv-1m`, `segment-mito-candidate-events`, `audit-mito-candidate-event-robustness`, `run-mito-event-random-benchmark`, `audit-mito-event-random-benchmark-breakdown` CLI 는 `mito0o852/OHLCV-1m` 월별 parquet directory 를 같은 반증 파이프라인으로 처리한다. 경로는 명시적 `--input-root`, `PSR_MITO_OHLCV_1M_PATH`, `PSR_DATA_ROOT/raw/huggingface/mito_ohlcv_1m/data`, repo-local fallback 순서로 해석한다. Mito random benchmark 는 random-control outcome join 을 기본 1개월 chunk 로 처리하고, breakdown 은 기존 benchmark 산출물을 event year/month/time/bucket 별로 분해한다. 산출물은 계속 `decision_grade=False`, `cost_grade=none` 이며 random benchmark 우위 없이는 setup backtest 금지 상태를 해제하지 않는다.
...
- Hugging Face CryptoSpartan 1m bars audit 산출물은 항상 `decision_grade=False`, `cost_grade=none` 이다. 이 데이터는 gross OHLCV falsification data 로만 쓰며 NBBO/spread/execution-cost blocker 를 해소하지 않는다.
...
## 2026-05-07 Mito OHLCV candidate-event 반증 결과

Mito OHLCV-only candidate event 는 gross coverage / 분산은 통과했지만 same ticker/day random event-time benchmark 를 이기지 못했다.
...
| `mito_event_random_breakdown_20260507_exact` | `breakdown_rows=1,296`, surviving pockets 0, breakdown gate FAIL |
