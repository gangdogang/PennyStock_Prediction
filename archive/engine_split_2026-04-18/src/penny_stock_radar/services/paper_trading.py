...
class PaperTradingEngine:
        ...
        *,
        ...
        if not rows and market_phase in {"premarket", "regular"}:
            scan_result = self.scanner.scan(
                phase=market_phase,
                top_limit=10,
                extra_symbols=self._active_position_symbols(),
            )
    ...
    def process_market_activity(
        ...
        *,
        ...
        self._latch_daily_loss_lock(run=run, positions=positions, market_date=market_date)
    ...
    def _apply_entry_rules(
        ...
        *,
        ...
        risk_cap = (
            equity_basis * self.settings.trade_plan_max_concurrent_open_risk_pct / 100.0
        )
        tracked_open_risk = current_open_risk(open_positions)
        concurrent_risk_locked = tracked_open_risk >= risk_cap
        if daily_loss_locked or concurrent_risk_locked:
            return []
        ...
        for position in open_positions:
            ...
            candidate_position.cost_basis += fill_price * quantity
    ...
    def _close_for_session_end(
        ...
        *,
        ...
        for position in positions:
            if position.status != "OPEN":
                continue
            if market_status_blocks_trading(self._position_market_status(position)):
                continue
            orders.append(
                self._close_position(
                    run=run,
    ...
    def _passes_adaptive_guardrails(
        ...
        *,
        ...
        if row.last_price is None or row.last_price <= 0:
            return False
        if not self._has_tradeable_live_state(row, market_phase=row.market_phase):
            return False
        ...
        *,
        ...
        if not rows and market_phase in {"premarket", "regular"}:
            extra_symbols = sorted(
                {
                    symbol
                    for engine in [self.primary_engine, *self.baseline_engines]
                    for symbol in engine._active_position_symbols()
                }
            )
