...
class TradePlanService:
        ...
        *,
        ...
        if not rows and market_phase in {"premarket", "regular"}:
            scan_result = self.scanner.scan(
                ...
                top_limit=10,
                persist=True,
                extra_symbols=tuple(sorted(open_symbols)),
            )
        ...
        current_day_pnl_value = current_day_pnl(now=now, positions=positions)
        equity_basis = run.equity if run is not None and run.equity > 0 else self.settings.paper_initial_capital
        daily_loss_locked = (
            has_daily_loss_lock(run.notes, market_date)
            if run is not None
            else False
        ) or current_day_pnl_value <= -(
            equity_basis * self.settings.trade_plan_daily_loss_lock_pct / 100.0
        )
        current_open_risk_value = current_open_risk(open_positions)
        concurrent_risk_locked = current_open_risk_value >= equity_basis * self.settings.trade_plan_max_concurrent_open_risk_pct / 100.0
        ...
        if export:
            ...
            self._write_checklist(
                ...
                regime=regime,
                daily_loss_locked=daily_loss_locked,
                current_day_pnl=current_day_pnl_value,
                current_open_risk=current_open_risk_value,
                generated_at=now,
    ...
    def _base_blockers(
        ...
        *,
        ...
        if not row.has_live_quote or not live_quote_is_valid(row.bid_price, row.ask_price):
    ...
    def _planned_stop(
        ...
        *,
        ...
        if entry_reference is None:
            return None
        if bucket == "open_position" and position is not None:
            return position.planned_stop_price or position.stop_price
    ...
    def _current_open_risk(self, positions: list[PaperPosition]) -> float:
        return current_open_risk(positions)
