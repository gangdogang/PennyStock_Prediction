...
def test_trade_plan_blocks_halted_market_status(tmp_path: Path) -> None:
    db_path = tmp_path / "radar.sqlite3"
    init_database(db_path)
    ...
    result = service.generate(
        ...
        activity=[
            _activity(
                ...
                score=4.4,
                predicted=True,
                market_status="tradingHalt",
            )
        ],
        export=False,
    )

    candidate = next(row for row in result.candidates if row.symbol == "HALT")
    assert candidate.actionability == "blocked"
    assert "market_halted" in candidate.blockers
...
def test_trade_plan_respects_latched_daily_loss_lock_note(tmp_path: Path) -> None:
    db_path = tmp_path / "radar.sqlite3"
    init_database(db_path)
...
def test_trade_plan_scan_includes_open_position_symbols(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "radar.sqlite3"
    ...
    def fake_scan(*, phase, top_limit, extra_symbols, **kwargs):
        assert phase == "premarket"
        assert extra_symbols == ("HOLD",)
    ...
    monkeypatch.setattr(service.scanner, "scan", fake_scan)

    result = service.generate(phase="premarket", activity=None, export=False)

    candidate = next(row for row in result.candidates if row.symbol == "HOLD")
    assert candidate.actionability == "blocked"
    assert "market_halted" in candidate.blockers
