...
def filter_universe(records: list[dict[str, Any]], config: Any) -> list[dict[str, Any]]:
    ...
    for raw_record in records:
        ...
        if price is None or not (
            _config_value(config, "price_min", 0.3)
            <= price
            <= _config_value(config, "price_max", 5.0)
        ):
            continue
        if market_cap is not None and market_cap > _config_value(
            config, "market_cap_max", 250_000_000
        ):
            continue
        if float_shares is not None and float_shares > _config_value(
            config, "float_shares_max", 10_000_000
        ):
            continue
