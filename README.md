...
.\scripts\psradar.ps1 audit-mito-event-random-benchmark-breakdown --run-id mito_random_breakdown_20260507
```

Mito 기본 경로는 `PSR_MITO_OHLCV_1M_PATH`, `PSR_DATA_ROOT\raw\huggingface\mito_ohlcv_1m\data`, repo-local fallback 순서다. 이 경로는 월별 parquet directory 입력을 받는다. random benchmark 는 random-control parquet join 을 기본 1개월 chunk 로 처리해 Windows 메모리 압박을 낮춘다. breakdown 은 기존 benchmark run 의 `random_events.csv` 를 event year/month/time/bucket 별로 분해한다. Mito 결과도 CryptoSpartan 과 같은 gross OHLCV 반증 산출물일 뿐이며, random benchmark 를 명확히 이기기 전에는 setup backtest 로 승격하지 않는다.
